...
import { AUTHENTICATE, HANDSHAKE, PROTOCOL_VERSION, HEARTBEAT_SIGNAL, HEALTH_CHECK } from "./CONSTANTS.js";
...
// FIXME: all connection.write operations return a boolean true if the data was flushed into the kernel else false(need handling) e.g line 43 and 95
// FIXME: the problem above is called: back‑pressure or flow control, do we buffer and retry?
// FIXME: (additional) not TLS, just raw handshake
const auth = new Auth()
...
function connectionData(data, connectionId, op){
        ...
        switch (op) {
            case -1:
                ...
                // FIXME: can become a bottleneck under concurrency, maybe an external store redis, or custom in memory store
            ...
            default:
                // FIXME: (advanced) we do everything in the main thread, maybe we should move this to a separate thread, offload to a worker threa
    ...
    connection(c) {
        try {
            ...
            // clients.set(key, {c, isAlive: true}); //FIXME: Use an array instead of an object (bad idea don't do tuples see  clients manager)
                //FIXME: a timer for each client connection, should consider a single timer that scans all clients once.
               const heartbeatInterval = setInterval(() => {
                ...
                // FIXME: returns a **boolean** true if the message was flushed successfully to the kernel else false meaning it queued: if all or part of the data was queued in user memory what can we do? is it a problem?
            ...
            c.on("data", (data)=> {
                ...
                if(op == HANDSHAKE){
                   ...
                   if(v != PROTOCOL_VERSION){
                       ...
                       // FIXME: we just destroy with no reason(Brute‑force guard) no throttle or ban repeated attempts(DDOS)
                        c.destroy();
                        globalclientsManager.remove(key);
                        clearInterval(heartbeatInterval)
                     return
