...
class QueueManager {
  ...
  async subscribe(clientId, queueName) {
    debugLib.Debug(`new sub: ${clientId}-> ${queueName}`)
    //const release = await this.globalMutex.acquire();
    const release = await this.getQueueMutex(queueName).acquire()
  ...
  scheduleDelivery(queueName) {
    debugLib.Debug(`schedule message: ${queueName}`)
  ...
  async createQueue(queueName, opts = {}) {
    ...
    if (resolvedOpts.Durable) this.durable.set(queueName, true);
