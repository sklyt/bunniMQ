...
// FIXME: TUPLES options everywhere
...
class QueueManager {
  ...
  /**
   * @type {Map<string, [ClassicDoublyLinkedList, Record<any, any>]>}  // FIXME: TUPLES (hey I thought js objects were slow with that prototype thing low level languages don't have, but it's code smell)
   */
  queues = new Map()
  durable = new Map()
  ...
  constructor() {
    this.timeoutMs = minutesToMilliseconds(DEFAULT_OPTS.queue.AckExpiry)
...
// TODO: solution a cleanup scheduler
    this.snapshotInterval = setInterval(() => this.takeSnapshot(), 60000); // every 30 seconds 4x the heartbeat check 
  ...
  subscribe(clientId, queueName) {
    ...
    if(!this.consumers.get(queueName).get(clientId)){
  ...
  restoreFromDisk() {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        debugLib.Debug(`no of queues: ${this.queues.size}`, "info")
       
        debugLib.Debug(`restoring queue: ${row.name}`, "info")
      
        this.queues.set(row.name, this.deserializeQueue(row.data));
        this.durable.set(row.name, "")
        debugLib.Debug(`no of queues: ${this.queues.size}`, "info")
    

      } catch (e) {
        console.error(`Failed to restore queue ${row.name}:`, e);
      }
    }
  }
...
const globalQueuepManager = new QueueManager() // FIXME: Inconsistent naming & typos extra p 
export default globalQueuepManager      // FIXME: Inconsistent naming & typos