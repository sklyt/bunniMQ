...
// legacy v0 protocol, New AMQP-inspired protocol soon
export function parseProtocol(data) {
  const parsed = {
    flags: data.readInt8(1),
  ...
  const metaStart = 6 + parsed.length;

    try {
      parsed.meta = parsed.flags === JSONENCODED 
        ? JSON.parse(data.subarray(metaStart).toString())
        : data.subarray(metaStart);
    } catch (error) {
      debugLib.Debug("Metadata parsing error", "error");
    }
