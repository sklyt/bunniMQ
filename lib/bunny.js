...
function setupQueueTimers(queueCleanupInterval, pendingMessagesTimeout){
    const timers = getTimeManager()
    const queueMan = getQueueManager(pendingMessagesTimeout)
...
export default function CreateBunny(opts = {}) {
  ...
  setupQueueTimers(config.queueCleanupInterval, config.queue.AckExpiry)
